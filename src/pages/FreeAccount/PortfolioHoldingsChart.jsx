...
const HOLDING_COLORS = [
  "#26457f","#4e84ff","#7aa56d","#7ab9ff","#b1965e",
  "#8c63d8","#b98bf2","#79a96a","#295da8","#ad6230",
  "#f3872d","#a8bfd9","#85b864","#488bb8","#f1ce4b",
  "#6ab5ff","#d9bfd9",
];
...
export default function PortfolioHoldingsChart({
  ...
  const holdingsGradient = useMemo(() => buildHoldingsGradient(topHoldings), [topHoldings]);
  ...
  return (
    <div className={wrapperClass}>
      ...
      <div className={`flex-1 flex flex-col items-center justify-center ${isLarge ? "py-4" : "py-3"}`}>
        {holdingsView === "donut" ? (
          hasPositions ? (
            <div className={`relative rounded-full shrink-0 ${isLarge ? "w-[340px] h-[340px]" : "w-[250px] h-[250px]"}`}>
              <div className="w-full h-full rounded-full" style={{ background: holdingsGradient }} />
              <div className={`absolute rounded-full bg-white dark:bg-gray-900 border border-[#edf1f7] dark:border-gray-700 ${isLarge ? "inset-[88px]" : "inset-[67px]"}`} />

              {topHoldings.map((item, index) => {
                const angle =
                  topHoldings.slice(0, index).reduce(
                    (sum, holding) => sum + Number(holding.weightPercent || 0), 0
                ...
                return (
                  <div
                    ...
                    className={`absolute font-semibold whitespace-nowrap ${isLarge ? "text-[14px]" : "text-[11px]"}`}
                    style={{
                      left: `${x}px`,
                      top: `${y}px`,
                  ...
                  >
                    {item.ticker} {formatCompactPercent(item.weightPercent, locale)}
                  </div>
                );
              })}
            </div>
            ...
            <div className={`relative rounded-full bg-[#f1f5f9] dark:bg-gray-800 border border-[#e2e8f0] dark:border-gray-700 flex items-center justify-center ${isLarge ? "w-[340px] h-[340px]" : "w-[250px] h-[250px]"}`}>
              <div className={`absolute rounded-full bg-white dark:bg-gray-900 border border-[#e2e8f0] dark:border-gray-700 ${isLarge ? "inset-[88px]" : "inset-[67px]"}`} />
